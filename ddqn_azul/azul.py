...
class Azul(object):
    def __init__(self, mode):
        ...
        if mode == "DDQN":
            self.deep_q = DeepQ(input_shape=(*self.env.observation_space.shape,
                                             1),
    ...
    def convert_process_buffer(self):
        ...
        black_buffer = [
            cv2.resize(cv2.cvtColor(x, cv2.COLOR_RGB2GRAY), (84, 90))
    ...
    def train(self, num_frames):
        ...
        while observation_num < num_frames:
            ...
            initial_state = self.env.get_obs()
            #self.process_buffer = []
            ...
            self.replay_buffer.add(initial_state, predict_movement, reward,
                                   done, new_state)
            total_reward += reward
